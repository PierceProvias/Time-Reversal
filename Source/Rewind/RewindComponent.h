...
{
	GENERATED_BODY();
	FTransformAndVelocitySnapshot() = default;
	FTransformAndVelocitySnapshot(float, FTransform, FVector, FVector);
...
{
	GENERATED_BODY();
	FMovementVelocityAndModeSnapshot() = default;
	FMovementVelocityAndModeSnapshot(float, FVector, TEnumAsByte<enum EMovementMode>);
	// Time since the last snapshot was recorded
	UPROPERTY(Transient)
	float TimeSinceLastSnapshot = 0.0f;
